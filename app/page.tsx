...
function GlobalAIBriefing() {
  ...
  return (
    <div className="bg-neutral-900 border border-neutral-800 rounded-2xl flex flex-col h-[350px] overflow-hidden">
      <div className="p-4 border-b border-neutral-800 shrink-0 bg-neutral-900 z-10 relative">
        <h3 className="text-sm font-medium text-neutral-400">AI 심층 분석 & 챗봇</h3>
      </div>
      
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 text-sm relative">
        {briefing === undefined ? (
          ...
          <>
            <div className="bg-neutral-800 text-neutral-200 p-4 rounded-2xl rounded-tl-sm self-start max-w-[95%] whitespace-pre-line leading-relaxed shadow-sm">
              <div className="font-bold mb-3 flex items-center gap-1.5 text-blue-400">
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {briefing.key_takeaways.map((kw: any, i: any) => (
                  <span key={i} className="px-2 py-0.5 bg-neutral-900 text-white rounded text-[10px] font-semibold tracking-wide border border-neutral-700">
                    {kw}
                  </span>
                ))}
              </div>
            ...
            {messages.map((m, i) => (
              <div key={i} className={`p-3 rounded-2xl max-w-[90%] whitespace-pre-line leading-relaxed shadow-sm ${m.role === "user" ? "bg-blue-600 text-white self-end rounded-tr-sm" : "bg-neutral-800 text-neutral-200 self-start rounded-tl-sm"}`}>
                {m.content}
              </div>
            ))}
            
            {isLoading && (
              <div className="bg-neutral-800 text-neutral-400 p-3 rounded-2xl rounded-tl-sm self-start max-w-[80%] flex items-center gap-2">
      ...
      <div className="p-3 border-t border-neutral-800 shrink-0 bg-neutral-900">
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            ...
            className="flex-1 bg-neutral-800 text-white rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 disabled:opacity-50"
          />
          <button 
            type="submit" 
    ...
    <div className="min-h-screen bg-black flex flex-col">
      ...
      <div className="p-6 lg:p-8 max-w-[1800px] w-full mx-auto flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-2">
          ...
          <section className="lg:col-span-1 bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden h-[350px] relative">
            <div className="absolute top-0 left-0 right-0 h-[48px] bg-neutral-900 z-10 flex items-center px-6 border-b border-neutral-800">
               <h3 className="text-sm font-medium text-neutral-400">주요 경제 캘린더</h3>
            </div>
          ...
          <section className="lg:col-span-1 bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden h-[350px] relative">
            <div className="absolute top-0 left-0 right-0 h-[48px] bg-neutral-900 z-10 flex items-center px-6 border-b border-neutral-800">
               <h3 className="text-sm font-medium text-neutral-400">실시간 글로벌 주요 뉴스</h3>
            </div>
            <div className="w-full h-[calc(100%+48px)] mt-[-48px]">
